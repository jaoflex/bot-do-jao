...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Flw  @${num.split('@')[1]}ja foi tarde vai fazer nem falta🥶👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ 👁to fazendo calma carai👁 ⌛',
				success: '✔️ ta feito de nada ✔️',
				error: {
					stick: '❌ n deu desculpas do bot😞🤝💓❌',
					Iv: '❌ Link invalido ❌'
				},
				only: {
					group: '❌ esse comando so pode ser usado em grupos❌',
					ownerG: '❌ so o dono pode parceiro ❌',
					ownerB: '❌ esse comando so pode ser usado pelo bot proprietario 😞❌',
					admin: '❌vc n é ademe💅😑😑😑❌',
					Badmin: '❌ vc n esta nesse patamar so adm pode🥶🤝💅❌'
				}
			}
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'blocklist':
					teks = 'pessoas que vieram encher o saco no pv:\n'
				...
				case 'dono':
					...
					buffer = await getBuffer(`file:///C:/Users/João%20Victor/Desktop/bd7fc96b-4234-4e8e-9e38-b10578bac411.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '(⁰̷¹̷⁰̷¹̷𝖏̷𝖆̷𝖔̷ ̷𝖋̷𝖑̷𝖊̷𝖝̷ NÚMERO: wa.me//557791235791/n INSTA:@jaorodrigues._)'})
					break
				case 'meme':
				...
				/*case 'loli':
					loli.getSFWLoli(async (err, res) => {
						if (err) return reply('❌ *deu erro tarado* ❌')
				...
				case 'nsfwloli':
					...
					loli.getNSFWLoli(async (err, res) => {
						if (err) return reply('❌ *deu erro taradao* ❌')
						buffer = await getBuffer(res.url)
